...
class MarriottUltimateSystem:
    def __init__(self, root):
        self.root = root
        self.root.title("Marriott v228 - Final Fixed Edition")
        ...
        for f in [DB_FILE, TRANS_FILE]:
            if not os.path.exists(f):
                with open(f, 'w', newline='', encoding='utf-8') as file:
                    header = self.all_cols if f == DB_FILE else ['ReqID', 'Date', 'Outlet', 'Subject', 'TotalValue', 'Status', 'Items']
    ...
    def setup_styles(self):
        ...
        style.configure("Treeview", background=CORE_UI["THEME"]["GRID_BG"], foreground="white", 
                        fieldbackground=CORE_UI["THEME"]["GRID_BG"], rowheight=25, font=("Segoe UI", 10))
    ...
    def show_login_screen(self):
        ...
        for txt, rcode in roles:
            btn = tk.Button(c, text=txt, font=("Arial", 11, "bold"), bg=CORE_UI["THEME"]["CARD"], fg="white", width=50, height=2, bd=0, 
    ...
    def auth_overlay(self, rcode):
        ...
        tk.Label(center, text=f"LOGIN AS {rcode}", fg="white", bg=CORE_UI["THEME"]["CARD"], font=("Arial", 12, "bold")).pack(pady=10)
        self.pwd_ent = tk.Entry(center, font=("Arial", 24), show="*", bg="#000", fg=CORE_UI["THEME"]["ON"], justify="center", width=12, insertbackground="white")
        self.pwd_ent.pack(pady=15, ipady=5); self.pwd_ent.focus_set()
        self.pwd_ent.bind("<Return>", lambda e: self.verify_login(rcode))
        tk.Button(center, text="CONFIRM", bg=CORE_UI["THEME"]["ON"], fg="white", font=("Arial", 11, "bold"), width=20, height=2, command=lambda: self.verify_login(rcode)).pack(pady=10)
    ...
    # ========================================================
    # [COST CONTROLLER - RE-ADDED ALL BUTTONS]
    # ========================================================
    def build_cost_controller_ui(self):
        self.clear_ui(); self.load_data()
        tk.Label(self.root, text="COST CONTROLLER - MASTER CONTROL", bg=CORE_UI["THEME"]["HEADER"], fg="white", font=("Arial", 12, "bold")).pack(fill="x", ipady=10)
        
        main_split = tk.Frame(self.root, bg=CORE_UI["THEME"]["BG"]); main_split.pack(fill="both", expand=True, padx=20)
        
        # LEFT: INVENTORY SECTION
        inv_f = tk.Frame(main_split, bg=CORE_UI["THEME"]["BG"]); inv_f.pack(side="left", fill="both", expand=True)
        btn_f = tk.Frame(inv_f, bg=CORE_UI["THEME"]["BG"], pady=10); btn_f.pack(fill="x")
        ...
        tk.Button(btn_f, text="📤 EXPORT MASTER", bg="#607D8B", fg="white", font=("Arial", 9, "bold"), command=lambda: self.export_inventory(False)).pack(side="left", padx=5)
        tk.Button(btn_f, text="📉 EXPORT BELOW PAR", bg="#E64A19", fg="white", font=("Arial", 9, "bold"), command=lambda: self.export_inventory(True)).pack(side="left", padx=5)
        tk.Button(btn_f, text="LOGOUT", bg="#444", fg="white", width=12, command=self.show_login_screen).pack(side="right")

        self.admin_tree = ttk.Treeview(inv_f, columns=self.all_cols, show='headings')
        for c in self.all_cols: 
            self.admin_tree.heading(c, text=c.upper()); self.admin_tree.column(c, width=100, anchor="center")
        ...
        # RIGHT: LIVE FEED
        live_f = tk.Frame(main_split, bg=CORE_UI["THEME"]["CARD"], width=350, padx=10); live_f.pack(side="right", fill="both")
        tk.Label(live_f, text="📡 LIVE TRANSACTION FEED", fg=CORE_UI["THEME"]["LIVE"], bg=CORE_UI["THEME"]["CARD"], font=("Arial", 10, "bold")).pack(pady=10)
        self.live_tree = ttk.Treeview(live_f, columns=('Date', 'Outlet', 'Status'), show='headings', height=20)
        for c in ('Date', 'Outlet', 'Status'): self.live_tree.heading(c, text=c); self.live_tree.column(c, width=100)
    ...
    def refresh_live_feed(self):
        ...
        try:
            with open(TRANS_FILE, 'r') as f:
                data = list(csv.DictReader(f))[-15:]
                for r in reversed(data):
                    self.live_tree.insert('', 'end', values=(r['Date'], r['Outlet'], r['Status']))
        except: pass
        if hasattr(self, 'root'): self.root.after(5000, self.refresh_live_feed) 
            ...
            try:
                with open(p, 'r', encoding='utf-8-sig') as f:
                    ...
                    for row in raw_data:
                        ...
                        s = self.safe_float(new_row.get('Stock On Hand', 0)); uc = self.safe_float(new_row.get('Unit cost', 0))
                ...
                self.refresh_admin_table(); messagebox.showinfo("Success", "Database Updated!")
            except Exception as e: messagebox.showerror("Error", str(e))

    def export_inventory(self, below_par_only=False):
        data = [r for r in self.inventory if self.safe_float(r.get('Stock On Hand', 0)) < self.safe_float(r.get('Min Par', 0))] if below_par_only else self.inventory
    ...
    # ========================================================
    # [CHEF DASHBOARD - RE-ADDED PRINT LOGIC]
    # ========================================================
    def build_chef_dashboard(self):
        ...
        tk.Label(self.detail_f, text="ITEMS (Press ENTER to Edit Qty)", fg=CORE_UI["THEME"]["ON"], bg=CORE_UI["THEME"]["CARD"], font=("Arial", 10, "bold")).pack(pady=10)
        ...
        # PRINT BUTTON RE-ADDED
        self.chef_print_btn = tk.Button(self.detail_f, text="🖨️ PRINT PREVIEW (A4)", bg="#1976D2", fg="white", font=("Arial", 10, "bold"), state="disabled", command=self.chef_print_action)
        self.chef_print_btn.pack(fill="x", pady=5)
        
        tk.Button(self.detail_f, text="✅ APPROVE THIS REQ", bg=CORE_UI["THEME"]["ON"], fg="white", font=("Arial", 11, "bold"), height=2, command=self.chef_approve_action).pack(fill="x", pady=5)
    ...
    def chef_edit_qty(self, event):
        ...
        new_q = simpledialog.askfloat("Chef Edit", f"Qty for {old['Desc']}:", initialvalue=old['Qty'])
        ...
        with open(TRANS_FILE, 'r') as f:
            ...
            for r in reader:
                if r['ReqID'] == rid:
                    ...
                    r['TotalValue'] = sum(i['Total'] for i in self.current_chef_items); final_rec = r
                rows.append(r)
        with open(TRANS_FILE, 'w', newline='') as f:
            dw = csv.DictWriter(f, fieldnames=fn); dw.writeheader(); dw.writerows(rows)
        messagebox.showinfo("Success", "Request Approved!"); self.chef_print_btn.config(state="normal"); self.last_approved_data = final_rec
        self.show_transaction_print(final_rec)

    def chef_print_action(self):
        if hasattr(self, 'last_approved_data'): self.show_transaction_print(self.last_approved_data)

    # ========================================================
    # [A4 PRINT VIEW - FULL LOGIC]
    # ========================================================
    def show_transaction_print(self, req_data):
        items = ast.literal_eval(req_data['Items'])
        pv = tk.Toplevel(self.root); pv.title("MARRIOTT - A4 PRINT VIEW"); pv.state('zoomed'); pv.configure(bg="#222")
        ...
        can.create_text(400, 75, text="INTERNAL STOCK REQUISITION", font=("Arial", 11, "bold", "underline"))
        can.create_text(60, 150, text=f"REQ ID: {req_data['ReqID']}", anchor="w", font=("Arial", 10, "bold"))
        can.create_text(60, 170, text=f"DATE: {req_data['Date']}", anchor="w")
        can.create_text(60, 190, text=f"OUTLET: {req_data['Outlet']}", anchor="w")
        
        y = 250; can.create_rectangle(50, y, 750, y+25, fill="#EEEEEE")
        headers = [("CODE", 60), ("DESCRIPTION", 150), ("UNIT COST", 480), ("QTY", 580), ("VALUE", 740)]
        for txt, x in headers: can.create_text(x, y+12, text=txt, anchor="w" if x < 400 else "e", font=("Arial", 9, "bold"))
        
        y += 35
        for i in items:
            can.create_text(60, y, text=i.get('Code','-'), anchor="w", font=("Courier", 9))
            can.create_text(150, y, text=str(i['Desc'])[:40], anchor="w", font=("Courier", 9))
            can.create_text(480, y, text=f"{i.get('Cost',0):,.2f}", anchor="e", font=("Courier", 9))
            can.create_text(580, y, text=f"{i['Qty']:.2f}", anchor="e", font=("Courier", 9))
            can.create_text(740, y, text=f"{i['Total']:,.2f}", anchor="e", font=("Courier", 9)); y += 22
            
        can.create_line(50, y+10, 750, y+10)
        can.create_text(740, y+40, text=f"GRAND TOTAL (LKR): {float(req_data['TotalValue']):,.2f}", anchor="e", font=("Arial", 12, "bold"))
        
        sig_y = 1000
        can.create_text(150, sig_y, text="..........................\nRequested By", justify="center")
        can.create_text(400, sig_y, text="..........................\nChef Approved", justify="center")
        can.create_text(650, sig_y, text="..........................\nIssued By Store", justify="center")
    ...
    def build_store_dashboard(self):
        self.clear_ui()
        tk.Label(self.root, text="STOREKEEPER - ISSUE CONTROL", bg=CORE_UI["THEME"]["HEADER"], fg="white", font=("Arial", 12, "bold")).pack(fill="x", ipady=15)
        ...
        self.load_transactions("CHEF APPROVED", t)
        
        def issue_stock():
            sel = t.selection()
            if not sel: return
            rid = t.item(sel[0], 'values')[0]
            if messagebox.askyesno("Confirm Issue", f"Issue Items for {rid}?"):
                self.process_store_issue(rid); messagebox.showinfo("Success", "Stock Issued!"); self.build_store_dashboard()
        
        tk.Button(self.root, text="📦 ISSUE STOCK (DEDUCT SOH)", bg=CORE_UI["THEME"]["ON"], fg="white", height=2, command=issue_stock).pack(pady=10)
    ...
    def build_outlet_selection_ui(self):
        self.clear_ui()
        tk.Label(self.root, text="CHOOSE YOUR KITCHEN", font=("Arial", 25, "bold"), bg=CORE_UI["THEME"]["BG"], fg="white").pack(pady=30)
    ...
    def setup_order_meta(self, name):
        ...
        self.sub_ent = tk.Entry(c, font=("Arial", 12), width=48); self.sub_ent.pack(pady=5)
    ...
    def build_outlet_grid(self):
        ...
        tk.Label(header, text=f"NEW ORDER: {self.req_id} | {self.selected_outlet}", fg="white", bg=CORE_UI["THEME"]["HEADER"]).pack()
    ...
    def submit_to_chef(self):
        ...
        messagebox.showinfo("Order Placed", "Sent to Chef."); self.show_login_screen()
        ...
        try:
            with open(TRANS_FILE, 'r') as f:
                for r in csv.DictReader(f):
                    if r['Status'] == status: tree.insert('', 'end', values=(r['ReqID'], r['Date'], r['Outlet'], r.get('Subject','-'), f"{self.safe_float(r['TotalValue']):,.2f}"))
        except: pass
